...
SQLControl::SQLControl(std::string password)
{
    if (loadConnection(password)) {
        ...
        std::cerr << "Warning: No sql config file exists. Please generate one now, configuration will auto-connect as soon as completed. \n";
    }
}
...
bool SQLControl::SQLSearch(std::vector<std::string> getColumns, std::vector<std::string> searchColumns, std::string table, std::string searchTerm, FilterPass filter, int limit, int offset, bool desc)
{
    ...
    try {
        ...
        if (!Bind(fil2) && (int)fil2.size() > 0) {
            std::cerr << "ERROR @ search Bind: Filter failed to bind. \n";
        }
...
bool SQLControl::SQLUpdate(std::string table, FilterPass updates, FilterPass filter)
{
    try {
        ...
        if (!Bind(updates) && (int)updates.size() > 0) {
            std::cerr << "ERROR @ Bind: Updates failed to bind. \n";
            return false;
        }
        /*
        for (int i{ 0 }; i < updates.size(); ++i) {
            std::cout << updates.at(i).first << ' ' << updates.at(i).second.str << ' ' << updates.at(i).second.num << ' ' << updates.at(i).second.dbl << '\n';
        }
        */
        if (m_stmt->execute()) {
            std::cerr << "ERROR @ UPDATE: failed to execute \n";
            return false;
        }
...
bool SQLControl::updateConfigTime(std::string name)
{
    try {
        ...
        if (!Bind(filter)) {
            std::cerr << "ERROR @ configtime Bind: Filter failed to bind. \n";
            return false;
        }

        if (m_stmt->execute()) {
            std::cerr << "ERROR @ UPDATE configtime: failed to execute \n";
            return false;
        }
    }
    catch (sql::SQLException& e) {
...
bool SQLControl::loadConnection(std::string password)
{
    ...
    if (!myfile) {
        std::cerr << "failed to read sql config file \n";
    ...
    std::cout << "Connection file loaded... \n";

    std::cout << m_user << ' ' << m_pass << ' ' << m_IP << ' ' << m_database << '\n';
